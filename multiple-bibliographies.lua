...
local supports_quiet_flag = (function()
    ...
    return major > 0 or minor > 14 or (minor == 14 and patch >= 5)
end)()

local function run_citeproc(doc, quiet)
    if PANDOC_VERSION >= "2.11" then
        return run_json_filter(doc, "pandoc", {"--from=json", "--to=json", "--citeproc", quiet and "--quiet" or nil})
    else
        -- doc = run_json_filter(doc, 'pandoc-citeproc')
        return run_json_filter(doc, "pandoc-citeproc", {FORMAT, (quiet and supports_quiet_flag) and "-q" or nil})
    end
end
