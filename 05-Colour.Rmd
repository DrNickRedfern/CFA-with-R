...
lch_data <- expand.grid(l = seq(0, 100, 20), c = seq(0, 100, 10), h = seq(0, 350, 10))
lch_rgb <- convert_colour(lch_data, from = "lch", to = "rgb")
lch_plot <- cbind.data.frame(lch_data, lch_rgb)
...
ggplot(data = lch_plot) +
  geom_rect(aes(xmin = h, xmax = h + resolution(h), ymin = c, ymax = c + resolution(c), 
                fill = rgb(r, g, b, maxColorValue = 255))) +
  coord_polar(theta = "x", start = 6.19592,  direction = 1) +
  facet_wrap(~l, labeller = as_labeller(l_labels)) +
  scale_x_continuous(breaks = seq(5, 355, 30), 
                     labels = c("0\u00B0", "30\u00B0", "60\u00B0", "90\u00B0", "120\u00B0", "150\u00B0",
                                "180\u00B0", "210\u00B0", "240\u00B0", "270\u00B0", "300\u00B0", "330\u00B0")) +
  scale_y_continuous(name = "Chroma (%)", breaks = seq(0, 100, 20), labels = seq(0, 100, 20)) +
  scale_fill_identity() +
  theme_dark() +
  theme(axis.title = element_text(size = 10),
        axis.text = element_text(size = 8),
        axis.text.x = element_text(angle = c(0, 330, 300, 270, 240, 210, 180, 150, 120, 90, 60, 30), 
                                   colour = "white"),
        strip.text.x = element_text(size = 8))
```
